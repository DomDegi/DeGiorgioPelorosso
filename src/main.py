...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Batch size for RAM-safe processing.",
    )
    parser.add_argument(
        "--input_path", type=str, required=True, help="Path to the input telemetry CSV."
    )
    parser.add_argument(
        "--output_path",
        type=str,
        required=True,
        help="Path to save output results and logs.",
    )
    parser.add_argument(
        "--rules_path",
        type=str,
        required=True,
        help="Path to the rules JSON configuration.",
    )
    parser.add_argument(
        "--sensors_path",
        type=str,
        required=True,
        help="Path to the sensors YAML configuration.",
    )
    ...
    reader = CSVTelemetryReader(
        sensors_yaml_path=args.sensors_path, csv_path=args.input_path
    )
