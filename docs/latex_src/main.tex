...
\vspace{0.3cm}
\noindent
To ensure architectural consistency and high document quality, the team adopted a strict peer-review methodology. Neither section was developed in isolation. Both members actively cross-validated each other's work, suggested logic modifications, refined terminology, and collaboratively managed the AI prompting process to finalize the formatting and UML standards.

\begin{table}[htbp]
    \centering
    \renewcommand{\arraystretch}{1.5}
    \begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{4cm} X >{\centering\arraybackslash}p{3cm} @{}}
        \toprule
        \textbf{Team Member} & \textbf{Task Description} & \textbf{Effort (Hours)} \\
        \midrule
        Domenico De Giorgio & \textbf{System Design, HPC Architecture \& DevOps Pipeline:} \newline
        Drafted the architectural descriptions, defined component behaviors, and generated UML diagrams. Engineered the high-performance Polars-based Rules Engine. Designed the multi-cloud automated CI/CD infrastructure, integrating cross-platform API observability, Singularity \texttt{.sif} container generation, and SLURM workload orchestration with NVMe \texttt{\$WORK} optimization. & 90 \\
        \midrule
        Leonardo Pelorosso & \textbf{Requirement Engineering, QA \& Virtualization:} \newline
        Identified system actors, functional/non-functional constraints, and detailed Use Case scenarios. Established reproducible Docker and DevContainer environments. Developed the rigorous \texttt{pytest} framework, managed CodeQL security scanning, and validated the mathematical constraints of the operational telemetry logic. & 90 \\
        \midrule
        Domenico De Giorgio \& \newline Leonardo Pelorosso & \textbf{Peer Review, Benchmarking \& AI Integration:} \newline
        Cross-validated architectural logic, refined domain terminology, and co-managed AI prompting for code optimization and quality compliance. Conducted system benchmarking and analyzed software scaling/performance on the Galileo100 SLURM platform. & 20 \par (10 hours each) \\
        \midrule
        & \multicolumn{1}{r}{\textbf{Total Project Effort}} & \textbf{200} \\
        \bottomrule
    \end{tabularx}
    \caption{Work distribution among team members and hours allocated per task.}
...
\begin{enumerate}[label=\textbf{Topic \arabic*:}, align=left, leftmargin=0.5cm]
    ...
    \begin{itemize}[label={}, leftmargin=0.5cm]
        \item \textbf{Tool Used:} Gemini Advanced
    ...
    \begin{itemize}[label={}, leftmargin=0.5cm]
        \item \textbf{Tool Used:} GitHub Copilot \& Gemini Advanced
