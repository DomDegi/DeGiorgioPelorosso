...
The most critical use case is \textbf{UC-4}, as it ensures the accurate
identification of nominal data and anomalies across all rule types.
...
\textbf{Use Case Name} & EvaluateValidityRules (UC-4) (Cont.) \\ \hline
\endhead

% Other pages footer
\hline
\multicolumn{2}{|r|}{\textit{Continued on next page\ldots}} \\ \hline
\endfoot
...
% Table body
\textbf{Actors} & AstraLog-HPC System, System Operator \\ \hline

\textbf{Description} &
The system evaluates a validated batch of telemetry data against all active
rules. It processes base rules, resolves cross-sensor correlation rules,
and finally partitions the data into valid telemetry and anomalies. \\ \hline

\textbf{Pre-conditions} &
Validation rules are successfully loaded and prioritized
(HIGH $>$ MEDIUM $>$ LOW). \\
& A structurally valid and sanitized batch of telemetry data is available
for processing. \\ \hline

\textbf{Trigger} &
The data extraction and sanitization phase successfully outputs a new data batch. \\ \hline

\textbf{Main Success} & \textbf{Phase 1 — Base Rule Evaluation:} \\
\textbf{Scenario} & \textbf{1.} The system isolates the data for each specific sensor defined in the base rules. \\
& \textbf{2a.} \textit{Simple:} The system evaluates the data against static threshold conditions; breaching measurements are flagged. \\
& \textbf{2b.} \textit{Step Difference:} The system evaluates the rate of change between consecutive readings. It retrieves the last known value from the previous batch to evaluate the first row, and persists the current batch's final value for future use. \\
& \textbf{2c.} \textit{Stateful:} The system tracks uninterrupted streaks of threshold breaches. It retrieves the ongoing streak count from the previous batch, increments it as necessary, and flags an anomaly if the defined consecutive limit is reached. The final streak state is persisted. \\
& \textbf{3.} The system records the identified anomalies for each evaluated base rule. \\
& \\ % Empty row for visual separation
& \textbf{Phase 2 — Correlation Rule Resolution:} \\
& \textbf{4.} The system processes correlation rules that depend on multiple base conditions. \\
& \textbf{5.} It retrieves the previously recorded anomaly timestamps for all dependent conditions. \\
& \textbf{6a.} \textit{AND logic:} The system retains only the timestamps where \textit{all} specified conditions are violated simultaneously. \\
& \textbf{6b.} \textit{OR logic:} The system retains the timestamps where \textit{at least one} of the specified conditions is violated. \\
& \textbf{7.} A unified anomaly record is generated for the correlated breach, aggregating the context of all involved sensors. \\
& \\
& \textbf{Phase 3 — Separation \& Deterministic Sorting:} \\
& \textbf{8.} The system aggregates all identified anomalies across all evaluated rules. \\
& \textbf{9.} \textit{Timestamp-level granularity:} Any timestamp containing at least one anomaly is entirely excluded from the valid telemetry dataset, ensuring strict temporal integrity. \\
& \textbf{10.} The generated anomaly records are sorted deterministically (by timestamp, priority, and identifiers) to guarantee reproducible reporting. \\ \hline

\textbf{Alternative} & \textbf{A1. Missing prior state for historical rules:} \\
\textbf{Flows} & If no historical data exists for Step or Stateful rules (e.g., during a cold start or first batch), the system initializes the evaluation with safe defaults (zero delta, zero streak) and proceeds normally. \\
& \\
& \textbf{A2. Invalid logic operator in Correlation rule:} \\
& If a correlation rule contains an unsupported logical operator during the loading phase, the system logs an error and discards the rule. The system continues operations with the valid rules. \\
& \\
& \textbf{A3. Correlation rule missing dependencies:} \\
& If a correlation rule lacks defined conditions at runtime, the system safely ignores it without generating false anomalies. \\
& \\
& \textbf{A4. Absent sensor data:} \\
& If a rule targets a sensor that is not present in the current batch, the system evaluates it as compliant (no violation) and continues processing the remaining rules without interruption. \\ \hline

\textbf{Post-conditions} & \textbf{Valid Telemetry:} A dataset containing only data points for timestamps where no rule violations occurred across any sensor. \\
& \\
& \textbf{Anomaly Records:} A deterministically sorted dataset containing detailed records of all identified anomalies, ready for downstream reporting or alerting. \\ \hline
...
\begin{enumerate}[label=\textbf{DA-\arabic*:}, align=left, leftmargin=2cm]
    ...
    \begin{itemize}[leftmargin=0.5cm]
        ...
        \item \textbf{Logical Correlation Rule:} Combines the Boolean outcomes of two or more other evaluated rules at the same timestamp using standard logical operators (AND / OR).
    \end{itemize}
...
\begin{enumerate}[label=\textbf{FR-\arabic*:}, align=left, leftmargin=2cm]
    ...
    \item system shall filter data packets checking their integrity and accumulate valid packets into a local batch\_file.
...
\begin{enumerate}[label=\textbf{CR-\arabic*:}, align=left, leftmargin=2cm]
    ...
    \begin{itemize}[leftmargin=0cm]
        ...
        \newpage
        \begin{verbatim}
        // correlation (accepting 2+ rules)
        {
            "rule_id": "str",
            "type": "correlation",
            "logic": "AND",        // AND | OR
            "conditions": ["rule_id_1", "rule_id_2", "rule_id_3"],
            "priority": "HIGH"
        }
        \end{verbatim}
...
\begin{enumerate}[label=\textbf{Topic \arabic*:}, align=left, leftmargin=0.5cm]
    ...
    \item \textbf{Codebase Review and Architectural Validation}
    \begin{itemize}[label={}, leftmargin=0.5cm]
    ...
    \begin{itemize}[label={}, leftmargin=0.5cm]
        ...
        \item \textbf{Integration:} Integrated into the CI pipeline and README, providing immediate, verifiable proof of the 96\% test coverage standard directly within the documentation.
    \end{itemize}
    \vspace{0.3cm}
    ...
    \item \textbf{RASD Gap Analysis and Documentation Review}
    \begin{itemize}[label={}, leftmargin=0.5cm]
        \item \textbf{Tool Used:} Claude Sonnet 4.6
        \item \textbf{Prompt Concept:} "Cross-reference the submitted RASD against the original assignment specification and identify structural gaps, inconsistencies, or missing elements that could affect the evaluation."
        \item \textbf{AI Output:} Identified a set of discrepancies between the document and the assignment requirements, including a missing \texttt{logic} field in the correlation rule format of CR-2, an inconsistency between the version number in the compiled PDF and the LaTeX source, and a mismatch in the effort hours between the two documents. It also flagged that FR-5 had been reformulated to implicitly reflect an implementation deviation that should instead be documented only in the Design section.
        \item \textbf{Integration:} Each finding was individually evaluated by the team. Confirmed issues were corrected directly in the LaTeX source: the \texttt{logic} field was added to the CR-2 correlation schema, version and effort figures were aligned, and FR-5 was restored to match the original assignment wording. The in-memory buffering deviation was retained exclusively in Section 3.3 as a documented design decision. Additionally, the tool was used to generate draft content for the UC-4 detailed table restructuring and the README Testing \& Rationale section, both of which were reviewed and adjusted before integration.
    \end{itemize}
    \vspace{0.3cm}
