...
ax1.set_ylabel('Total Execution Time (Seconds)', color=color1, fontsize=12, fontweight='bold')
line1 = ax1.plot(df['Batch Size'], df['Total Time (s)'], marker='o', color=color1, linewidth=2.5, label='Total Time (s)')
...
ax1.annotate(f'Sweet Spot\n({int(min_batch/1000)}k rows)', xy=(min_batch, min_time), xytext=(min_batch, min_time + 2),
...
color2 = '#d62728' # Red
ax2.set_ylabel('Peak Memory Required (MB)', color=color2, fontsize=12, fontweight='bold')
line2 = ax2.plot(df['Batch Size'], df['Max RAM (MB)'], marker='s', color=color2, linewidth=2.5, linestyle='--', label='Max RAM (MB)')
...
plt.title('Performance vs. Batch Size\n(The U-Shaped Curve & Memory Wall)', fontsize=14, fontweight='bold', pad=15)
fig.tight_layout()

# Save the plot
plt.savefig('benchmark_plot_galileo100_32CPU_64GB.png', dpi=300)
print("Plot successfully generated and saved as benchmark_plot_galileo100_32CPU_64GB.png")