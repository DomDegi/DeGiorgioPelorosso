...
def generate_massive_dataset(
    num_rows: int = 1_000_000, output_path: str = "input/export_sat_alpha_massive.csv"
):
    ...
    anomaly_indices = np.random.choice(
        num_rows, size=int(num_rows * 0.01), replace=False
    )
    value_col[anomaly_indices] = np.random.choice(
        [60.0, 10.0, -5.0, 100.0], size=len(anomaly_indices)
    )
