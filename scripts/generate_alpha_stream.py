...
def generate_mission_dataset(
    sensors_path: str, rules_path: str, output_path: str, num_timestamps: int = 100_000
):
    ...
    for rule in rules:
        if rule["type"] in ["simple", "stateful"]:
            ...
            anomaly_idx = np.random.choice(
                target_sensor_idx,
                size=int(len(target_sensor_idx) * 0.02),
                replace=False,
            )
    ...
    for idx in error_indices:
        ...
        if error_type == "schema_missing":
            df.loc[idx, "priority"] = pl.NA  # Missing a field
        else:
    ...
    with open(output_path, "a") as f:
        f.write(
            "2026-05-01T23:59:58Z,TEMP-001,25.5\n"
        )  # Missing priority (schema error)
        f.write(
            '2026-05-01T23:59:59Z,PRES-002,101.3,"HIGH\n'
