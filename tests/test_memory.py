...
def test_count_namespace_isolation(memory):
    ...
    assert (
        memory.get_current_count("R3", "VOLT-MAIN") == 5
    ), "R3 count was incorrectly altered"
    assert (
        memory.get_current_count("R4", "VOLT-MAIN") == 2
    ), "R4 count was incorrectly altered"
...
def test_count_overwrite_updates_value(memory):
    ...
    assert (
        memory.get_current_count("R1", "TEMP-01") == 8
    ), "Memory failed to overwrite previous count"
...
def test_value_overwrite_updates_correctly(memory):
    ...
    assert (
        memory.get_last_value("TEMP-MAIN") == 48.9
    ), "Memory failed to update to the latest float value"
