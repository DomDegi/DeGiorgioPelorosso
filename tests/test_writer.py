...
def test_writer_clean_start(temp_output_dir):
    ...
    CSVOutputWriter(output_path=temp_output_dir, clean_start=True)
    assert not os.path.exists(
        valid_path
    ), "Writer did not remove old files on clean start"
...
def test_write_alarms_missing_columns(temp_output_dir, caplog):
    ...
    assert not os.path.exists(
        writer.alarms_file_path
    ), "Should not write corrupted data to disk"
