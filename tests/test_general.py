...
"""
    ...
    reader = CSVTelemetryReader(
        sensors_yaml_path=str(sensors_path), csv_path=str(input_csv_path)
    )
    ...
    assert (
        alarms_log_file.exists()
    ), "alarms.log was not created in the output directory"
    assert (
        valid_data_file.exists()
    ), "valid_data.csv was not created in the output directory"
    ...
    assert (
        len(alarms_content) == 1
    ), f"Expected exactly 1 alarm, but found {len(alarms_content)}"
    assert (
        alarms_content[0] == expected_alarm
    ), f"Alarm format mismatch. Got: {alarms_content[0]}"

    expected_nominal = "2026-05-01T23:59:59Z;NOMINAL;PRES-002:101.3"
    assert (
        len(valid_data_content) == 1
    ), f"Expected exactly 1 valid data entry, but found {len(valid_data_content)}"
    assert (
        valid_data_content[0] == expected_nominal
    ), f"Valid data format mismatch. Got: {valid_data_content[0]}"
